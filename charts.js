...
function initializeAllCharts() {
    ...
    console.log('中国地图是否已注册:', echarts.getMap && echarts.getMap('china') ? true : false);
    ...
    if (echarts.getMap && !echarts.getMap('china')) {
        console.log('中国地图未加载，尝试从本地文件加载');
        // 尝试使用本地自定义地图
        const script = document.createElement('script');
        script.src = 'china_map.js';
        script.onload = function() {
            console.log('本地中国地图加载完成，开始初始化地图');
            // 延迟初始化地图，确保地图数据已加载
    ...
    try {
        if (document.querySelector('#china-map .chart-container')) {
            ...
            if (!echarts.getMap('china')) {
                console.error('中国地图数据未注册，尝试重新注册');
                registerFallbackChinaMap();
            }
...
// 简化版注册中国地图
function registerFallbackChinaMap() {
    console.log('注册简化版中国地图');
    ...
    const features = provinces.map((name, index) => {
        ...
        return {
            ...
            geometry: {
                type: 'Polygon',
                coordinates: [[[x, y], [x+0.9, y], [x+0.9, y+0.9], [x, y+0.9], [x, y]]]
            }
        };
    });
